...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    ...
    {
        title: t('leads.edit_title'),
        href: `/leads/${props.lead.id}/edit`,
    },
]);

const form = useForm({
    lead_form_id: String(props.lead.lead_form_id),
    name: props.lead.name,
    email: props.lead.email,
    phone: props.lead.phone,
    source: props.lead.source,
    payload_json: JSON.stringify(props.lead.payload ?? {}, null, 2),
});
...
const submit = (): void => {
    ...
    form.transform((data) => ({
        ...
        payload,
    })).patch(`/leads/${props.lead.id}`);
};
</script>

<template>
    <Head :title="t('leads.edit_title')" />
